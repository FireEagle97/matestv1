...
class PaymentController extends Controller
{
    ...
    protected function PaystackPayment(Request $request)
    {
        ...
        $validCurrencies = ['NGN', 'GHS', 'ZAR', 'USD'];
    
        // Log detected currency
        Log::info("Detected currency: $currency");
    
        // Validate currency
        if (!in_array($currency, $validCurrencies)) {
            Log::warning("Invalid currency detected: $currency. Defaulting to NGN.");
            $currency = 'NGN';
        }
    
        // Check if Paystack allows USD for this account
        if ($currency === 'USD' && !$this->isUSDAllowedByPaystack()) {
            Log::warning("USD not supported by this Paystack account. Defaulting to NGN.");
            $currency = 'NGN';
        }
    
        // Log final currency being used
        Log::info("Final currency for payment: $currency");
    
        // Create a new Paystack payment request
        ...
        Log::info('Paystack API Response:', $responseBody);
